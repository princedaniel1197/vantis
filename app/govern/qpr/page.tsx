    ...
    <div className="flex flex-col min-h-full text-off-white">
      ...
      <div className="px-6 sm:px-8 py-5 border-b border-border shrink-0">
        <div className="text-[9px] font-mono uppercase tracking-[0.28em] text-gray mb-2">
          K-RERA · Karnataka Real Estate Regulatory Authority · Compliance Monitoring
        </div>
        <h1 className="font-syne text-2xl sm:text-3xl font-bold text-off-white leading-none">
          QPR Compliance Tracker
        </h1>
      </div>

      <div className="px-6 sm:px-8 py-6">

        <p className="text-gray text-xs mb-6">Quarterly Progress Reports · {qprData.quarters.length} quarters tracked</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          ...
          >
            <div className="font-syne text-2xl sm:text-3xl font-bold text-off-white">{currentRows.length}</div>
          ...
          >
            <div className="font-syne text-2xl sm:text-3xl font-bold text-green">{onTimeCount}</div>
          ...
          >
            <div className="font-syne text-2xl sm:text-3xl font-bold text-red">{defaultingCount}</div>
        ...
        {totalPenalty > 0 && (
          <div className="bg-red/5 border border-red/20 rounded-sm p-4 sm:p-5 mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
        ...
        <div className="hidden md:block bg-surface border border-border rounded-sm overflow-hidden mb-4">
          <table className="w-full">
            ...
            <tbody>
              {displayRows.filter(r => r.entry.status !== 'NA').map((row, index) => {
          ...
          {displayRows.filter(r => r.entry.status !== 'NA').map((row, index) => {
            ...
            return (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.04, duration: 0.35, ease: [0.33, 1, 0.68, 1] }}
                className={`border rounded-sm p-3 hover:border-gold/30 transition-all ${
