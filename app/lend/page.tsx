...
import { BAND_COLOR, BAND_LABEL, type LendProject, type RiskBand } from '@/lib/lend-portfolio'
import { useLendContext } from './LendContext'
...
function Section({
  ...
  band: RiskBand; count: number; exposure: number; projects: LendProject[]; defaultOpen: boolean
}) {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray text-[10px] font-mono">
            {count} projects · ₹{exposure.toLocaleString('en-IN')} Cr
          </span>
        </div>
        <ChevronDown
      ...
      {open && (
        <div className="border-t border-border">
          <div className="hidden md:flex items-center gap-4 px-4 py-1.5 bg-surface2 border-b border-border">
            ...
            <div className="w-16 text-right text-[9px] font-mono uppercase tracking-[0.12em] text-gray shrink-0">Outstanding</div>
...
export default function LendPortfolioDashboard() {
  const { projects, personaMeta } = useLendContext()

  const red   = projects.filter(p => p.risk_band === 'red')
  const amber = projects.filter(p => p.risk_band === 'amber')
  const green = projects.filter(p => p.risk_band === 'green')

  const redExposure   = red.reduce((s, p) => s + p.outstanding_cr, 0)
  const amberExposure = amber.reduce((s, p) => s + p.outstanding_cr, 0)
